...
function classifyKey(key) {
  ...
  if (!t) return 'empty';
  if (t.startsWith('xai-') && t.length > 12) return 'xai';
...
function pickApiKey() {
  const a = String(process.env.AI_API_KEY || '').trim();
  const b = String(process.env.XAI_API_KEY || '').trim();
...
function publicFromUpstream(status, data) {
  ...
  if (status === 401 || status === 403) {
    return 'xAI 拒绝了密钥。请确认 Render 的 AI_API_KEY 是 console.x.ai 里完整的 xai- 开头密钥（不是团队 ID），保存后等重新部署。';
  }
