...
function resolveAiConfig(settings) {
  const provider = (process.env.AI_PROVIDER || (settings && settings.provider) || 'openai').toLowerCase();
  const apiKey = process.env.AI_API_KEY || '';
  const imageKey = process.env.IMAGE_API_KEY || apiKey;
  const apiBaseUrl = process.env.AI_API_BASE || (provider === 'gemini'
  ...
  return {
    ...
    imageModel,
    configured: !!apiKey,
    imageConfigured: !!imageKey && provider !== 'none',
