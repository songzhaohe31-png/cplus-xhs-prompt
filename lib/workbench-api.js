...
const { resolveAiConfig, callChat, generateImage, AiError, keyHint } = require('./ai');
...
module.exports = function attachWorkbench(ctx) {
  ...
  app.post('/api/agent/chat', async (req, res) => {
    ...
    if (!cfg.configured || !cfg.keyLooksValid) {
      const err = keyHint(cfg) || PUBLIC_AI_ERR;
      addLog({ kind: 'chat', user: 'public', status: 'unconfigured', intent, message, model: '' });
    ...
    const packed = buildContext({
      ...
      knowledge: knowledgeHits.length ? knowledgeHits : mine(req, lists.knowledge.items),
      feed: mine(req, lists.feed.items),
      contents: mine(req, lists.contents.items),
      materials: mine(req, lists.materials.items),
      message
    });
    let official = [];
    try {
    ...
    try {
      ...
      ], settings(), { timeoutMs: 70000, retries: 0, maxTokens: 2200 });
      const parsed = parseAgentReply(reply, intent);
      record.status = 'ok';
      lists.chat.items.push(record);
      persistList('chat');
      addLog({ kind: 'chat', user: 'public', status: 'ok', intent, message, model: '' });
      res.json({
        success: true,
        intent,
        reply: parsed.visible,
        structured: parsed,
        sources: (packed.sources || []).map((s) => ({ name: s.name, type: s.type })).concat(
          official.filter((o) => o.ok).map((o) => ({ type: 'official', name: o.name, url: o.url }))
        ),
        duplicates: packed.duplicates,
        dupHint: packed.dupHint
      });
    } catch (e) {
      addLog({ kind: 'chat', user: 'public', status: 'error', intent, message, model: '' });
      record.status = 'error';
      lists.chat.items.push(record);
      persistList('chat');
      console.error('[chat]', e && e.code, e && e.message);
      const err = (e && e.publicMessage) || PUBLIC_AI_ERR;
