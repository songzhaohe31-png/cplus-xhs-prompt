...
function flattenVision(json, raw) {
  const parts = [];
  if (json && json.title) parts.push(String(json.title));
  if (json && json.text) parts.push(String(json.text));
  if (json && Array.isArray(json.points) && json.points.length) parts.push(json.points.map(String).join('\n'));
  if (json && json.layout) parts.push('版式：' + json.layout);
  if (json && json.logo) parts.push('Logo：' + json.logo);
  const text = parts.filter(Boolean).join('\n').replace(/```json|```/g, '').trim();
  if (text) return text;
  return String(raw || '').replace(/```json|```/g, '').trim();
}

function posterFallback(originalName, extra) {
  const name = originalName || 'poster';
  return {
    ok: true,
    text: '',
    charCount: 0,
    ...
    titles: titlesFrom(name.replace(/\.[^.]+$/, '')),
    ocrUsed: false,
    ocrNote: extra || '图片已作为海报样本保存。',
    error: '',
    facts: [],
    visual: {
      type: 'poster',
...
async function extractImage(filePath, mime, originalName) {
  try {
    const { describeImage } = require('./ai');
    const { extractJson } = require('./parse-output');
    const vis = await describeImage(filePath, mime);
    const json = extractJson(vis.content) || {};
    const text = flattenVision(json, vis.content);
    const charCount = text.replace(/\s/g, '').length;
    const kind = json.type || 'poster';
    return {
      ok: true,
      text,
      charCount,
      pageCount: 1,
      pages: [{ page: 1, text, charCount }],
      titles: json.title ? [json.title].concat(titlesFrom(text)) : titlesFrom(text),
      ocrUsed: true,
      ocrNote: charCount
        ? (kind === 'poster' ? '已识别海报文字和版式。' : '已识别图片文字。')
        : '已读取图片，未见清晰文字，已作为海报样本保存。',
      error: '',
      facts: factsFrom(text, originalName || path.basename(filePath), 1),
      visual: {
        type: kind,
        colors: json.colors || [],
        layout: json.layout || '',
    ...
    const note = (e && e.publicMessage) || (e && e.message) || '请稍后重试';
    console.error('[extractImage]', note);
    return posterFallback(originalName, '图片已作为海报样本保存。文字识别未完成：' + note);
  }
}
...
async function extractDocument(filePath, mime, originalName) {
  ...
  try {
    ...
    if (/^\.(png|jpe?g|webp|gif|heic|heif|bmp)$/.test(ext) || kind.startsWith('image/')) {
      return extractImage(filePath, kind, originalName);
    }
