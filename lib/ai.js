...
function resolveAiConfig(settings) {
  ...
  return {
    ...
    firstTokenMs: envNum('XAI_FIRST_TOKEN_MS', 90000),
    ...
    const req = https.request(requestOpts(parsed, 'POST', { Authorization: 'Bearer ' + cfg.apiKey }, payload), (res) => {
      ...
      const firstTimer = setTimeout(() => {
        if (!firstTokenAt && !settled) {
          ...
          reject(new AiError('TIMEOUT', 'first token timeout', { publicMessage: 'AI请求超时，请重试。' }));
        }
      }, firstTokenMs);
      const totalTimer = setTimeout(() => {
        if (!settled) {
          ...
          reject(new AiError('TIMEOUT', 'total timeout', { publicMessage: 'AI请求超时，请重试。' }));
        }
      }, totalMs);
...
async function callChat(messages, settings, options) {
  const result = await callChatStream(messages, settings, options);
