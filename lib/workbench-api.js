...
const { resolveAiConfig, callChat, callChatStream, generateImage, AiError, keyHint, estTokens } = require('./ai');
...
module.exports = function attachWorkbench(ctx) {
  ...
  app.post('/api/agent/chat', async (req, res) => {
    ...
    try {
      ...
      const parsed = parseAgentReply(reply, packed.intent);
  ...
  app.post('/api/agent/chat/stream', async (req, res) => {
    ...
    sseWrite(res, 'meta', { intent: packed.intent });
    ...
    try {
      const result = await callChatStream([
        ...
        maxTokens: packed.intent === 'month' ? 1200 : (packed.intent === 'single' ? 1600 : cfg.maxTokens),
        signal: ac.signal,
        onDelta: (delta) => {
          sseWrite(res, 'delta', { text: delta });
        }
      });
      const parsed = parseAgentReply(result.text, packed.intent);
      addLog({ kind: 'chat', user: 'public', status: 'ok', intent: packed.intent, message, model: '' });
      sseWrite(res, 'done', {
        intent: packed.intent,
        reply: parsed.visible,
      ...
      addLog({ kind: 'chat', user: 'public', status: 'error', intent: packed.intent, message, model: '' });
      console.error('[chat/stream]', e && e.code, e && e.message);
      sseWrite(res, 'error', { error: (e && e.publicMessage) || PUBLIC_AI_ERR });
    } finally {
      clearInterval(ping);
      busy.delete('chat:' + ws);
      res.end();
  ...
  startJobRunner({
    ...
    runJob: async (job) => {
      ...
      const parsed = parseAgentReply(reply, 'single');
      return {
