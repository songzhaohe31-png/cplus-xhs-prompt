...
const { emptyDna, computeReadiness, dnaToPrompt, analyzePrompt, SERIES, DNA_FIELDS, isFact } = require('./dna');
...
module.exports = function attachWorkbench(ctx) {
  ...
  app.post('/api/dna/confirm', (req, res) => {
    const cur = workspaceDna(req);
    if (!cur.analyzedAt) return res.status(400).json({ error: '请先完成风格分析。' });
    const saved = saveDna(req, { ...cur, confirmed: true, confirmedAt: new Date().toISOString() });
  ...
  app.post('/api/dna/analyze', async (req, res) => {
    ...
    if (!facts.length && !samples.length && !feed.length) {
      return res.status(400).json({ error: '请先上传公司资料、历史文案或海报样本。' });
    }
    try {
      const reply = await callChat([
        { role: 'system', content: '你是CPLUS内容风格分析师。只输出JSON，不要解释。不要编造费用或监管数字。' },
        { role: 'user', content: analyzePrompt(facts, samples, feed) }
      ...
      const next = {
        ...
        confirmed: false,
        fields: { ...emptyDna().fields, ...(json.fields || {}) },
        copy: json.copy || {},
        visual: json.visual || {},
  ...
  function packChat(req, message) {
    ...
    const hits = searchChunks(
      (lists.chunks.items || []).filter((c) => c.workspaceId === ws || c.workspaceId === 'global' || !c.workspaceId),
      message,
      6
    ).slice(0, 4);
    ...
    const packed = buildContext({
      ...
      knowledge: knowledgeHits.length ? knowledgeHits : mine(req, lists.knowledge.items).slice(0, 4),
  ...
  function gateFormal(req, intent, generalMode) {
    ...
    if (ready.facts === 0) {
      return '缺少相关业务资料，暂时不能生成正式内容。请先上传相关服务资料或官方来源。';
    }
    if (!ready.analyzed || !ready.confirmed) {
      return '尚未建立账号风格。可以先上传3至5篇历史文案及海报，或使用通用模式生成测试稿。';
    }
    return '请先完成资料上传并确认账号风格，再生成正式内容。';
  }
  ...
  app.post('/api/agent/chat/stream', async (req, res) => {
    ...
    try {
      ...
      const parsed = parseAgentReply(result.text, packed.intent, packed.slots);
  ...
  app.get('/api/knowledge', (req, res) => {
    res.json({
      items: mine(req, lists.knowledge.items).map((k) => ({
        ...
        status: k.excerpt ? '资料已准备好' : '资料处理失败'
      }))
    });
  });
  ...
  app.post('/api/knowledge', async (req, res) => {
    ...
    res.json({ success: true, item: { id: item.id, name: item.name, status: item.excerpt ? '资料已准备好' : '资料处理失败' }, items: mine(req, lists.knowledge.items) });
  });
  ...
  app.delete('/api/knowledge/:id', (req, res) => {
    ...
    res.json({ success: true, items: mine(req, lists.knowledge.items) });
  });
