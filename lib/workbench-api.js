...
const { resolveAiConfig, callChat, callChatStreamWithRetry, generateImage, AiError, keyHint, estTokens, pickFastChatModel } = require('./ai');
...
const { emptyDna, computeReadiness, dnaToPrompt, analyzePrompt, SERIES, DNA_FIELDS, isFact, classifyUpload, captionStats, applyMix, materialFingerprint, countPosters, posterNotesFrom, buildLocalAnalysis } = require('./dna');
...
module.exports = function attachWorkbench(ctx) {
  ...
  function maybeInvalidateDna(req) {
    const knowledge = mine(req, lists.knowledge.items);
    const feed = mine(req, lists.feed.items);
    const cur = workspaceDna(req);
    const fp = materialFingerprint(knowledge, feed);
    if (cur.analyzedAt && cur.materialKey && cur.materialKey !== fp) {
      saveDna(req, { ...cur, stale: true });
    }
  ...
  app.post('/api/dna/analyze', async (req, res) => {
    ...
    const samples = knowledge.filter((k) => k.zone === 'sample');
    const feed = mine(req, lists.feed.items);
    const stats = captionStats(feed.map((f) => f.caption).concat(samples.map((s) => s.caption || s.excerpt)));
    const posterCount = countPosters(knowledge, feed);
    const posterNotes = posterNotesFrom(knowledge);
    if (!facts.length && !stats.n) {
      return res.status(400).json({ error: '请先上传公司资料或历史文案。' });
    }
    let brand = {};
    try { brand = require('../config/cplus-brand-rules.json'); } catch (e) { brand = {}; }
    const local = buildLocalAnalysis({ facts, samples, feed, stats, posterCount, posterNotes, brand });
    let json = {};
    let source = 'local';
    const cfg = resolveAiConfig(settings());
    if (cfg.configured && cfg.keyLooksValid) {
      try {
        const reply = await callChat([
          { role: 'system', content: '你是CPLUS内容风格分析师。只输出JSON。数字必须使用用户给出的程序统计。没有足够海报时禁止编造视觉规格。尽快完成。' },
          { role: 'user', content: analyzePrompt(facts, samples, feed, stats, posterCount, posterNotes) }
        ], settings(), {
          timeoutMs: 16000,
          firstTokenMs: 10000,
          maxTokens: 1200,
          reasoningEffort: 'low',
          noRetry: true,
          model: pickFastChatModel(cfg.model) || cfg.model
        });
        const { extractJson } = require('./parse-output');
        json = extractJson(reply) || {};
        if (json.fields && Object.keys(json.fields).length) source = 'ai';
      } catch (e) {
        console.warn('[dna/analyze] ai enrich skipped', (e && e.publicMessage) || e.message);
      }
    }
    try {
      const cur = workspaceDna(req);
      const fields = { ...local.fields, ...(json.fields || {}) };
      let visual = { ...local.visual, ...(json.visual || {}) };
      if (posterCount < 3) {
        fields.posterColors = '尚未上传海报，无法分析视觉风格。';
        fields.posterLayout = '尚未上传海报，无法分析视觉风格。';
        fields.imageElements = '尚未上传海报，无法分析视觉风格。';
        visual = { insufficient: true, note: '尚未上传海报，无法分析视觉风格。', n: posterCount };
      }
      const next = {
        ...cur,
        ...
        confirmed: false,
        stale: false,
        materialKey: materialFingerprint(knowledge, feed),
        source,
        fields,
        copy: {
          ...local.copy,
        ...
        visual,
        strategy: { ...local.strategy, ...(json.strategy || {}) },
        evidence: { captions: stats.n, posters: posterCount, facts: facts.length }
      };
      const saved = saveDna(req, next);
      res.json({ success: true, dna: saved, stats, source, readiness: readinessOf(req) });
    } catch (e) {
  ...
  app.get('/api/knowledge', (req, res) => {
    res.json({
      items: mine(req, lists.knowledge.items).map((k) => ({
        ...
        timely: !!k.timely,
        analyzed: k.status === 'ok' && ((k.charCount || 0) > 0 || k.kind === 'poster'),
  ...
  async function finishParse(item, abs) {
    ...
    const isImage = /^image\//.test(item.mime || '') || (doc.visual && doc.visual.type);
    if (!doc.ok && !isImage) {
      item.status = 'failed';
    ...
    item.charCount = doc.charCount || 0;
    item.pageCount = doc.pageCount || 1;
    ...
    item.ocrNote = doc.ocrNote || (!doc.ok ? (doc.error || '') : '');
