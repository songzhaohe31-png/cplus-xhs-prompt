...
async function callChatStream(messages, settings, options) {
  ...
  if (opts.reasoningEffort) bodyObj.reasoning_effort = opts.reasoningEffort;
  if (opts.temperature != null) bodyObj.temperature = opts.temperature;
  else if (!/^grok-4/.test(String(model))) bodyObj.temperature = 0.5;
...
function pickFastChatModel(preferred) {
  const skip = /imagine|tts|whisper|embed|vision|image|video|voice|audio/i;
  const chat = availableModels.filter((id) => id && !skip.test(id));
  const fast = chat.find((id) => /fast|mini|flash|lite/i.test(id) && !/reason/i.test(id));
  return fast || preferred || chat[0] || '';
}
...
async function callChatStreamWithRetry(messages, settings, options) {
  ...
  const fast = opts.noRetry ? '' : fasterFallback(primary);
...
function sniffImageMime(buf, fallback) {
  ...
  const brand = buf.slice(4, 16).toString('ascii');
  if (/ftyp(heic|heix|heif|mif1|msf1)/i.test(brand)) return 'image/heic';
  const fb = String(fallback || '');
  if (fb.startsWith('image/')) return fb;
  return 'image/jpeg';
}

async function prepareVisionImage(filePath, mime) {
  const fs = require('fs');
  let buf = fs.readFileSync(filePath);
  if (!buf.length) throw new AiError('EMPTY', 'empty image', { publicMessage: '图片文件是空的。' });
  let mimeType = sniffImageMime(buf, mime);
  let sharp = null;
  try { sharp = require('sharp'); } catch (e) { sharp = null; }
  if (sharp) {
    try {
      let img = sharp(buf, { failOn: 'none', animated: false, limitInputPixels: 40000000 }).rotate();
      ...
      console.warn('[vision] convert failed', e.message);
    }
  }
  if (mimeType !== 'image/jpeg' && mimeType !== 'image/png') {
    throw new AiError('UPSTREAM', 'unsupported image type ' + mimeType, {
      publicMessage: '目前只能识别JPG和PNG。请把图片另存为JPG后再上传。'
    });
  }
  if (buf.length > 18 * 1024 * 1024) {
    throw new AiError('UPSTREAM', 'image too large', { publicMessage: '图片太大，请压缩后重试。' });
  }
  return {
    mimeType,
    bytes: buf.length,
...
function pickVisionText(json) {
  if (!json || typeof json !== 'object') return '';
  if (typeof json.output_text === 'string' && json.output_text.trim()) return json.output_text;
  const choice = json.choices && json.choices[0];
  const msg = choice && (choice.message || choice.delta);
  const fromChoice = collectText(msg && msg.content) || collectText(msg && msg.reasoning_content);
  if (fromChoice.trim()) return fromChoice;
  const outputs = json.output || json.outputs || [];
  const fromOut = collectText(outputs);
  return String(fromOut || '').trim();
}
...
function visionModels(cfg) {
  ...
  const listed = availableModels.slice();
  const fast = listed.find((id) => /fast|mini/i.test(id) && !/imagine|tts|embed|video|voice/i.test(id));
  add(fast);
  add(cfg.model);
  add('grok-4.6');
  add('grok-4');
  add('grok-2-vision-1212');
  return ids;
}
...
function chatVisionPayload(model, dataUrl, prompt) {
...
function responsesVisionPayload(model, dataUrl, prompt) {
  return JSON.stringify({
    model,
    store: false,
...
async function describeImage(filePath, mime) {
  const cfg = resolveAiConfig({});
  assertKeyUsable(cfg);
  const prepared = await prepareVisionImage(filePath, mime);
  const prompt = visionPrompt();
  const models = visionModels(cfg).slice(0, 2);
  const attempts = [];
  if (models[0]) {
    attempts.push({
      model: models[0],
      url: chatCompletionsUrl(cfg.apiBaseUrl),
      payload: chatVisionPayload(models[0], prepared.dataUrl, prompt),
      kind: 'chat'
    });
    attempts.push({
      model: models[0],
      url: responsesUrl(cfg.apiBaseUrl),
      payload: responsesVisionPayload(models[0], prepared.dataUrl, prompt),
      kind: 'responses'
    });
  }
  if (models[1]) {
    attempts.push({
      model: models[1],
      url: chatCompletionsUrl(cfg.apiBaseUrl),
      payload: chatVisionPayload(models[1], prepared.dataUrl, prompt),
      kind: 'chat'
    });
  }
  let lastErr;
  for (let i = 0; i < attempts.length; i++) {
    ...
    try {
      console.log('[vision] try model=' + attempt.model + ' via=' + attempt.kind + ' bytes=' + prepared.bytes);
      const res = await requestJson('POST', attempt.url, { Authorization: 'Bearer ' + cfg.apiKey }, attempt.payload, 90000);
      if (res.status !== 200) {
        const hint = String(res.data || '').replace(/xai-[A-Za-z0-9_-]+/g, 'xai-***').slice(0, 240);
        console.error('[vision] upstream', res.status, attempt.model, attempt.kind, hint);
        lastErr = new AiError('UPSTREAM', 'vision ' + res.status, {
          status: res.status,
          publicMessage: publicFromUpstream(res.status, res.data) || '图片识别失败。'
        });
        if (res.status === 401 || res.status === 402 || res.status === 403) throw lastErr;
        continue;
      }
      let json;
      try { json = JSON.parse(res.data); } catch (e) {
        lastErr = new AiError('EMPTY', 'vision json', { publicMessage: '图片识别返回无法解析。' });
        continue;
      }
      const content = pickVisionText(json);
      if (!content) {
        lastErr = new AiError('EMPTY', 'vision empty', { publicMessage: '图片识别没有返回文字。' });
        continue;
      }
      console.log('[vision] ok model=' + attempt.model + ' via=' + attempt.kind + ' chars=' + String(content).length);
      return { model: attempt.model, content: String(content) };
    } catch (e) {
      lastErr = e instanceof AiError ? e : new AiError('NETWORK', e.message, { publicMessage: '网络连接失败。' });
      if (e.status === 401 || e.status === 402 || e.status === 403) throw lastErr;
      if (e.code === 'TIMEOUT' || e.code === 'NETWORK' || (e.status && e.status >= 500) || e.code === 'EMPTY' || e.code === 'UPSTREAM') {
        continue;
      }
    }
  }
