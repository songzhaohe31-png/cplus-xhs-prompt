...
async function extractImage(filePath, mime, originalName) {
  try {
    ...
    const text = String(json.text || vis.content || '').replace(/```json|```/g, '').trim();
    const charCount = text.replace(/\s/g, '').length;
    const kind = json.type || 'poster';
    const ok = charCount >= 4 || kind === 'poster' || kind === 'screenshot';
    return {
      ok,
      text,
      charCount,
      ...
      ocrUsed: true,
      ocrNote: kind === 'poster' ? '已识别海报文字和版式。' : '已识别图片文字。',
      error: ok ? '' : '图片中没有识别到有效文字。',
    ...
    return {
      ...
      ocrUsed: false,
      ocrNote: '',
      error: '图片识别失败：' + ((e && e.publicMessage) || (e && e.message) || '请稍后重试'),
      facts: []
    };
  }
}
...
async function extractDocument(filePath, mime, originalName) {
  ...
  try {
    ...
    if (/^\.(png|jpe?g|webp|gif)$/.test(ext) || kind.startsWith('image/')) return extractImage(filePath, kind, originalName);
