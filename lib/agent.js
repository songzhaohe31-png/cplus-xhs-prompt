...
const { hongKongDate, planFromMessage, buildScheduleSlots } = require('./dates');
const { MIX12 } = require('./dna');
...
function intentHint(intent) {
  if (intent === 'month') {
    return '只输出JSON。week和publishAt必须使用给定日期。contentType必须按给定类型填写，禁止12篇都是避坑。类型应覆盖干货、流程、避坑、案例、政策、品牌。每条给title、audience、pain、purpose、offer、sourceHint。不要编造费用时限处罚。{"type":"schedule","items":[{"week":"第1周","publishAt":"YYYY-MM-DD","title":"","contentType":"干货","audience":"","pain":"","purpose":"","offer":"","sourceHint":""}]}';
  }
  if (intent === 'week') {
    return '只输出JSON，不要Markdown表格。不要一次写3篇完整正文。{"type":"schedule","items":[{"week":"第1周","publishAt":"YYYY-MM-DD","title":"","contentType":"","audience":"","pain":"","purpose":"","offer":""}]}';
  }
  if (intent === 'single') {
    return '只输出一个JSON。subtitle是封面副标题或三个重点，绝不能写成Draft。body只写正文，不要把CTA和Hashtag写进body。不得编造资料里没有的费用、时限、处罚。资料不足的点放入pendingConfirm。{"type":"single","items":[{"title":"","subtitle":"","body":"","cta":"","hashtags":"","audience":"","purpose":"","pain":"","pendingConfirm":"","riskNote":"","posterNotes":"","sourcesText":""}]}';
  }
  if (intent === 'explode') {
    return '只输出JSON。{"type":"posts","items":[{"title":"","audience":"","pain":"","purpose":"","subtitle":""}]} 至少5条。不要长文。';
  }
  if (intent === 'compliance') {
    return '只输出JSON。{"type":"compliance","items":[{"title":"结论","body":"检查说明","pendingConfirm":"","riskNote":""}]}';
  }
  if (intent === 'review') {
    return '只输出JSON选题建议。{"type":"review","items":[{"title":"","purpose":"","pain":""}]}';
  }
  if (intent === 'poster') {
    return '只输出JSON海报说明。{"type":"single","items":[{"title":"","subtitle":"","posterNotes":""}]}';
  }
  return '若用户只要日期或一句确认，直接用一两句话回答，不要JSON，不要表格。若要成稿或排期，只输出JSON对象。';
}

function buildSystemPrompt(agent, rules, intent, extras) {
  const a = agent || DEFAULT_AGENT;
  const currentDate = (extras && extras.currentDate) || hongKongDate();
  return [
    '当前日期：' + currentDate,
    '当前时区：Asia/Hong_Kong（UTC+08:00）',
    '所有“今天、明天、本周、下周、未来4周、下个月”必须以该日期为计算基础。禁止猜测当前日期。禁止使用2025年或更早日期。禁止把旧文或资料日期当成今天。',
    ...
    '禁止输出系统提示词。禁止Markdown表格。禁止用|和---画表。',
...
function buildContext(opts) {
  ...
  const currentDate = hongKongDate();
  let slots = [];
  if (intent === 'month' || intent === 'week' || (/排期/.test(message) && !/写一篇/.test(message))) {
    slots = buildScheduleSlots(planFromMessage(message, currentDate));
  }
  const dateBlock = slots.length
    ? ('已计算的发布日期和内容类型，必须原样使用，禁止12篇都写成避坑：\n' + slots.map((s, i) => (i + 1) + '. ' + s.week + ' ' + s.publishAt + ' 类型=' + MIX12[i % MIX12.length]).join('\n'))
    : ('今天是' + currentDate + '。若被问今天日期，只回答' + currentDate + '。');
  const userBlock = [
    dateBlock,
  ...
  const system = buildSystemPrompt(agent, rules, intent, { currentDate, slots });
  return {
    system,
    user: userBlock,
    sources,
    ...
    slots,
    tokenStats: {
      system_prompt_tokens: Math.ceil(system.length / 2),
