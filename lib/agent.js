...
function intentHint(intent) {
  if (intent === 'month') {
    return '只输出JSON对象，不要Markdown，不要表格，不要竖线|，不要代码围栏，不要解释。x.ai只填写title/contentType/audience/pain/purpose/offer。week和publishAt必须使用用户消息里已给出的日期，禁止改日期，禁止使用2025年。{"type":"schedule","items":[{"week":"第1周","publishAt":"YYYY-MM-DD","title":"","contentType":"避坑","audience":"","pain":"","purpose":"","offer":""}]}';
  }
  if (intent === 'week') {
  ...
  if (intent === 'single') {
    return '只输出一个JSON对象，不要Markdown表格，不要代码围栏，不要解释。{"type":"single","items":[{"title":"","subtitle":"","body":"","cta":"","hashtags":"","audience":"","purpose":"","pain":"","pendingConfirm":"","riskNote":"","posterNotes":""}]} body用\\n分段，300-550字。';
  }
  if (intent === 'explode') {
...
function buildContext(opts) {
  ...
  const dateBlock = slots.length
    ? ('已计算的发布日期，必须原样写入每条item的week与publishAt，禁止修改：\n' + slots.map((s, i) => (i + 1) + '. ' + s.week + ' ' + s.publishAt).join('\n'))
